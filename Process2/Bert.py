...
tokenizer = AutoTokenizer.from_pretrained(r"B2")
...
class BertClassifier(nn.Module):
    def __init__(self, dropout=0.5):
        ...
        self.bert = AutoModel.from_pretrained(r'B2', output_hidden_states=True)
        self.dropout = nn.Dropout(dropout)
        self.linear = nn.Linear(768, 24)
        self.relu = nn.ReLU()

    def forward(self, input_id, mask):
        pooled_output = self.bert(input_ids=input_id, attention_mask=mask, return_dict=False)[0]["pooler_output"]
